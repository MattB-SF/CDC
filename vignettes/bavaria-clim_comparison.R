...
dwd_yearmon_tamm_bav_tk4tel <- dwd_yearmon_tamm_bav_tk4tel %>% group_by(x,y) %>% 
  mutate_at(vars(-group_cols()), .funs=function(x){x / 10})
...
dwd_yearmon_tadnmm_bav_tk4tel <- dwd_yearmon_tadnmm_bav_tk4tel %>% group_by(x,y) %>% 
  mutate_at(vars(-group_cols()), .funs=function(x){x / 10})
...
dwd_yearmon_tadxmm_bav_tk4tel <- dwd_yearmon_tadxmm_bav_tk4tel %>% group_by(x,y) %>% 
  mutate_at(vars(-group_cols()), .funs=function(x){x / 10})
...
## ---- eval=F------------------------------------------------------------------
#  ## Plot of tmean
#  baywrf_tmean <- baywrf_tas_bav %>% tidyr::pivot_longer(cols=-c(x,y), names_to="date", values_to="baywrf") %>%
#    filter(date %in% c("Mär.1980", "Jun.1990", "Sep.2000", "Dez.2010"))
#  dwd_yearmon_tamm_bav_tk4tel <- dwd_yearmon_tamm_bav_tk4tel %>% group_by(x,y) %>%
#    mutate_at(vars(-group_cols()), .funs=function(x){x / 10})
#  dwd_tmean <- dwd_yearmon_tamm_bav_tk4tel %>%
#    tidyr::pivot_longer(cols=-c(x,y), names_to="date", values_to="dwd") %>%
#    filter(date %in% c("Mär 1980", "Jun 1990", "Sep 2000", "Dez 2010"))
#  eobs_tmean <- `e-obs_tg_ens_mean_0.1deg_bav_yearmon` %>%
...
#    mutate_at(vars(-group_cols()), .funs=function(x){x - 273.15})
#  wfde5_tmean <- wfde5_tair_bav  %>%
#    tidyr::pivot_longer(cols=-c(x,y), names_to="date", values_to="wfde5")  %>%
#    filter(date %in% c("Mär 1980", "Jun 1990", "Sep 2000", "Dez 2010"))
#  baywrf_tmean$date <- sub(pattern="[.]", replacement=" ", x=baywrf_tmean$date)
#  
#  #head(baywrf_tmean)
...
#  all_tmean$date <- zoo::as.yearmon(all_tmean$date, "%B %Y")
#  
#  all_tmean %>% ggplot(aes(x=x, y=y,colour=tmean)) +
#    facet_grid(Dataset~date, scales="free") + geom_point() + #scale_fill_(drop=F) +
#    theme_bw()
