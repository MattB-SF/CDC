...
knitr::opts_chunk$set(
  ...
  eval=F, echo=T, error=F, fig.path="../figures/"
)

## ----pa-overview, fig.width=6, fig.height=8-----------------------------------
#  # Load CDC & ggplot2 package
#  library(CDC); library(dplyr); library(ggplot2); library(tidyr)
#  
#  # Load land-cover data from CDC package
#  data(cci_chi, package="CDC")
...
#  
#  cci_chi$x <- signif(cci_bav$x, digits=5)
#  cci_chi$y <- signif(cci_bav$y, digits=5)
#  df_area$x <- signif(df_area$x, digits=5)
#  df_area$y <- signif(df_area$y, digits=5)
#  cci_chi <- left_join(cci_bav, df_area)
#  
#  sum_cci <- cci_chi %>% gather(year, class, -c(x,y,area)) %>%
